import { View, Text, Image, Keyboard, TouchableOpacity, TextInput, Pressable, Dimensions, Modal, Platform, Alert } from "react-native";
...
const AddCommentButton = ({ réels, partVisible }) => {
  ...
  const handleComment = async () => {
    try {
      ...
      if ((userData._id && text && !mediaType) || (userData._id && !text && mediaType) || (userData._id && text && mediaType)) {
          ...
          >
            ...
            <TextInput
              ...
              placeholder={`${t('TextInputPost')} ${!isEmpty(usersData[0]) &&
                (() => {
                  const user = usersData.find((user) => user._id === réels.posterId);
